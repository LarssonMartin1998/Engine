...
bool D3D::Initialize(int screenWidth, int screenHeight, bool vSync, HWND hwnd, bool fullScreen, float screenDepth, float screenNear)
{
	...
	// Create a list to hold all the possible display modes for this monitor/video card comination.
	...
	for (unsigned displayModeIndex = 0; displayModeIndex < numModes; ++displayModeIndex)
	{
		DXGI_MODE_DESC& displayMode = displayModeList[displayModeIndex];

		if (displayMode.Width == (unsigned)screenWidth)
		{
			if (displayMode.Height == (unsigned)screenHeight)
			{
	...
	// Release structures and interfaces necessary to retrieve the numerator and denominator for the refresh rate and the video card information.
	...
	// Set the feature level of DirectX11.
	...
	// Create the rasterizer state from the description from the information that was just filled out.
	...
	orthographicMatrix = DirectX::XMMatrixOrthographicLH((float)screenWidth, (float)screenHeight, screenNear, screenDepth);
	...
	// Creat ethe state using the device
	result = device->CreateDepthStencilState(&depthDisabledStencilDesc, &depthStencilStateDisabled);
	if (FAILED(result))
	{
		return false;
	}
...
void D3D::Shutdown()
{
	// Before shutting down set to windowed mode, or else, when you release the swap chain it will throw and exception.
	if (swapChain)
	{

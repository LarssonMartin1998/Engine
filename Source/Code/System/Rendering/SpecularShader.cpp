...
SpecularShader::SpecularShader(const SpecularShader& /*specularShader*/)
{
...
bool SpecularShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;

	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, hwnd, L"specular_vertex.vs", L"specular_pixel.ps");
	if (!result)
	{
		return false;
	}
...
bool SpecularShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, DirectX::XMFLOAT3& cameraPosition, DirectX::XMMATRIX& worldMatrix, DirectX::XMMATRIX& viewMatrix, DirectX::XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, DirectX::XMFLOAT3& lightDirection, DirectX::XMFLOAT4& diffuseColor, DirectX::XMFLOAT4& ambientColor, float specularPower, DirectX::XMFLOAT4& specularColor)
{
	bool result;

	// Set the shader parameters that it will use for rendering.
	result = SetUpVertexShaderParameters(deviceContext, cameraPosition, worldMatrix, viewMatrix, projectionMatrix);
	if (!result)
	{
		return false;
	}

	result = SetUpPixelShaderParameters(deviceContext, texture, lightDirection, diffuseColor, ambientColor, specularPower, specularColor);
...
bool SpecularShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	HRESULT result = D3DCompileFromFile(vsFilepath, NULL, NULL, "SpecularVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
	...
	result = D3DCompileFromFile(psFilepath, NULL, NULL, "SpecularPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
	...
	// Set up the description of the dynamic camera constant buffer that is in the vertex shader.
	...
	// Set up the description of the dynamic light constant buffer that is in the pixel shader.
...
bool SpecularShader::SetUpVertexShaderParameters(ID3D11DeviceContext* deviceContext, DirectX::XMFLOAT3& cameraPosition, DirectX::XMMATRIX& worldMatrix, DirectX::XMMATRIX& viewMatrix, DirectX::XMMATRIX& projectionMatrix)
{
	...
	// Get a pointer to the data in the constant buffer.
	CameraBufferType* cameraData = (CameraBufferType*)mappedResource.pData;

	// Copy the camera position to the constant buffer.
	cameraData->cameraPosition = cameraPosition;
	cameraData->pad01 = 0.0f;
...
bool SpecularShader::SetUpPixelShaderParameters(ID3D11DeviceContext* deviceContext, ID3D11ShaderResourceView* texture, DirectX::XMFLOAT3& lightDirection, DirectX::XMFLOAT4& diffuseColor, DirectX::XMFLOAT4& ambientColor, float specularPower, DirectX::XMFLOAT4& specularColor)
{
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mappedResource;
...
void SpecularShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
...
void SpecularShader::CatchShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* filename)
{
...
void SpecularShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileError;
	size_t bufferSize;
	std::ofstream fout;
